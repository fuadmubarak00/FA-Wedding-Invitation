...
function getGalleryViewUrl(originalSrc, width) {
  ...
  return `/.netlify/images?url=${encodeURIComponent(originalSrc)}&w=${width}&fm=avif&q=75`;
}
  ...
  return <main>
    ...
    <div className={opened ? "site visible" : "site"}>
      ...
      <section className="gallery section-pad"><p className="eyebrow">Kenangan yang tersimpan</p><h3>Galeri Kami</h3><div className="photo-grid">
        {galleryPhotos.map((photo, index) => <button key={photo.originalSrc} className={`photo ${photo.className}`} type="button" onClick={() => setSelectedPhoto(photo)} aria-label={`Lihat ${photo.alt}`}><img src={getGalleryViewUrl(photo.originalSrc, 800)} srcSet={getGallerySrcSet(photo.originalSrc)} sizes={photo.sizes} width="6000" height="4000" alt={photo.alt} loading={index === 0 ? "eager" : "lazy"} fetchPriority={index === 0 ? "high" : "auto"} decoding="async" onError={(event) => { event.currentTarget.style.display = "none"; }}/><span></span></button>)}
      </div>
      ...
      <section className="gift section-pad"><Icon name="gift"/><p className="eyebrow">Tanda kasih</p><h3>Wedding Gift</h3><p>Doa restu Anda merupakan hadiah terindah bagi kami. Namun jika ingin memberikan tanda kasih, dapat melalui:</p><div className="bank-grid"><div><small>Bank Central Asia</small><strong>7790343772</strong><span>a.n. Armaningtyas Utami</span><button onClick={()=>copy("7790343772","BCA")}>{copied==="BCA"?"Tersalin ✓":"Salin Nomor"}</button></div><div><small>Bank Jago</small><strong>100265762269</strong><span>a.n. Armaningtyas Utami</span><button onClick={()=>copy("100265762269","Jago")}>{copied==="Jago"?"Tersalin ✓":"Salin Nomor"}</button></div></div></section>
